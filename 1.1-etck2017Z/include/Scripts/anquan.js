$(function() {
    var bobo = $(window).height();
    $('#banner .bg').css('height', bobo);
});
...
function AnimateBanner(parent, bg_d_T, i_d_T, o_d_T, c_T) {
    this.parent = parent, this.bg_d_T = bg_d_T, this.i_d_T = i_d_T, this.o_d_T = o_d_T, this.c_T = c_T, this.movement = null, this.init = function() {
        ...
        $("#banner .pr-info li.haibo_01 .i-o").add("#banner .pr-info li.haibo_01 .i-s").add("#banner .bg .haibo_01").css({
            "opacity": "1"
        });
    }, this.animate = function(prev, curr) {
        var that = this;
...
anquan.index = {
    ...
    bannerAction: new AnimateBanner($("#banner"), 500, 800, 800, 7000),
    init: function() {
        this.showCont();
        this.resize();
        this.posHeader();
    ...
    playAD: function(open_T, close_T, stop_T, c_T) {
        ...
        $(".ad_xm .min").click(function(e) {
            ...
            window.open('http://anquan.baidu.com/zt/mi?supply=50199', '_blank');
        });
    ...
    openNavPanel: function(target, panel) {
        ...
        $(panel).mouseleave(function(e) {
            if (e.relatedTarget.nodeName != "A" || !$(e.relatedTarget).parent().hasClass("haibo_01")) {
...
$(function() {
    function animatepic() {
        $(".animatepic").each(function() {
            var rowOffsetTop = $(this).offset().top;
            if ($(document).scrollTop() > rowOffsetTop - $(window).height() + 160) {
                $(this).find("li").addClass("zoomin");
            }
        })
    }

    $('#h_kjcd ul li').hover(function() {
        $(this).find('dl').animate({ top: '-269px', opacity: '100' }, 500)
    }, function() {
        $(this).find('dl').animate({ top: '0', opacity: '0' }, 500)
    })

    $('.h_zspt_box ul li').hover(function() {
        $(this).find('.nr_up').animate({ top: '0' }, 300);
        $(this).find('.nr_dow').animate({ top: '-140' }, 300);
        $(this).find('.nr_dow .a2').animate({ "padding-top": "30" }, 300);
    }, function() {
        $(this).find('.nr_up').animate({ top: '-154' }, 300);
        $(this).find('.nr_dow').animate({ top: '0' }, 300);
    })

    $('#h_pic ul li').hover(function() {
        $(this).find('p').animate({ "bottom": "0" }, 300);
        $(this).find('img').animate({ "top": "-40px" }, 300);
    }, function() {
        $(this).find('p').animate({ "bottom": "-40px" }, 300);
    ...
    $(window).scroll(function() {
        ...
        if ($(window).scrollTop() >= 2200) {
            $('#h_about .a2').animate({ 'left': '145px' }, 800);
            $('#h_about .a3').animate({ 'left': '287px' }, 800);
        }

        if ($(window).scrollTop() >= 400) {
            for (var i = 0; i < $('.km_left ul li').length; i++) {
                var top = i * 56;
                var time = i * 200;
                $('.km_left ul li').eq(i).animate({ 'top': top }, time);
            }
        }
    });

    $('.kbj-header-link li').hover(function() {
        $(this).find('.nav_hover').css('display', 'block');
    }, function() {
        $(this).find('.nav_hover').css('display', 'none');
    })
})
...
window.onload = function() {
    var nav = document.getElementById('daohang_nav');
